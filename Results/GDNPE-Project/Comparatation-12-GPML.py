################################################################################
## 对比实验12: Projection Metric Learning on Grassmann Manifold 效果测试
################################################################################
# 添加路径和消除警告
import sys
import warnings
...
for dn in config.GDNPE_data:
    ...
    pred_path = os.path.join("GPML-pred", str(epoch), dn + "-pred-" + str(epoch) + ".mat")
    sets = loadmat(pred_path)
    model.Grassmann_Contrast_Object.space = None
    model.Grassmann_Contrast_Object.time = None
    model.Grassmann_Contrast_Object.t_pred = sets.get("pred").flatten()
    model.Grassmann_Contrast_Object.target_test = sets.get("true").flatten()
