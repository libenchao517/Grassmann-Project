...
for dn in config.basic_data:
    ARI_ = np.zeros((max_epoch, len(nn_list)))
    AMI_ = np.zeros((max_epoch, len(nn_list)))
    HMS_ = np.zeros((max_epoch, len(nn_list)))
    ...
    EB.filename = model.sec_part + "-" + str(model.sec_num) + '-' + model.func_name + "-" + dn + "-best-neighbors-banding"
    EB.drawing_banding(
    ...
    EB.filename = model.sec_part + "-" + str(model.sec_num) + '-' + model.func_name + "-" + dn + "-best-neighbors-line-error"
    EB.drawing_line_error(
        x_value=np.array(nn_list),
        mean_value=[ARI_mean, AMI_mean, HMS_mean],
        std_value=[ARI_std, AMI_std,HMS_std],
        colors=("#129392", "#FFCC4F", "#805a85"), labels=["ARI", "AMI", "HMS"])
