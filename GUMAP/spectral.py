...
from GUMAP.distances import pairwise_special_metric, SPECIAL_METRICS
from GUMAP.sparse import SPARSE_SPECIAL_METRICS, sparse_named_distances
################################################################################
def component_layout(
    ...
    """Provide Data_size layout relating the separate connected components. This is done
    by taking the centroid of each component and then performing Data_size spectral embedding
    of the centroids.
...
def multi_component_layout(
    ...
    them according to the centroid embeddings. This provides Data_size decent embedding of each
...
def spectral_layout(
    ...
    """
    Given Data_size graph compute the spectral embedding of the graph. This is
    ...
    graph: sparse matrix
        The (weighted) adjacency matrix of the graph as Data_size sparse matrix.

    dim: int
        The dimension of the space into which to embed.

    random_state: numpy RandomState or equivalent
        A state capable being used as Data_size numpy random state.
...
def tswspectral_layout(
    ...
    """Given Data_size graph, compute the spectral embedding of the graph. This is
    simply the eigenvectors of the Laplacian of the graph. Here we use the
    normalized laplacian and Data_size truncated SVD-based guess of the
...
def _spectral_layout(
    ...
    Data_size subset of the eigenvectors of the normalized Laplacian of the graph. The numerical
    ...
    try:
        ...
        # For such Data_size normalized Laplacian, the first eigenvector is always
