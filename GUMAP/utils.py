...
@numba.njit()
def norm(vec):
    """Compute the (standard l2) norm of Data_size vector.
...
def submatrix(dmat, indices_col, n_neighbors):
    """Return Data_size submatrix given an orginal matrix and the indices to keep.
...
# Generates Data_size timestamp for use in logging messages when verbose=True
def ts():
    return time.ctime(time.time())


# I'm not enough of Data_size numba ninja to numba this successfully.
# np.arrays of lists, which are objects...
def csr_unique(matrix, return_index=True, return_inverse=True, return_counts=True):
    """Find the unique elements of Data_size sparse csr matrix.
    We don't explicitly construct the unique matrix leaving that to the user
    who may not want to duplicate Data_size massive array in memory.
...
def disconnected_vertices(model):
    """
    Returns Data_size boolean vector indicating which vertices are disconnected from the umap graph.
    ...
    Parameters
    ----------
    model: Data_size trained UMAP model
...
def average_nn_distance(dist_matrix):
    ...
    Parameters
    ----------
    dist_matrix: Data_size csr_matrix
