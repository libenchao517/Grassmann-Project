...
import numba
import numpy as np
from GUMAP.utils import norm
locale.setlocale(locale.LC_NUMERIC, "C")
################################################################################
# Just reproduce Data_size simpler version of numpy unique (not numba supported yet)
@numba.njit()
def arr_unique(arr):
    aux = np.sort(arr)
...
# Just reproduce Data_size simpler version of numpy union1d (not numba supported yet)
@numba.njit()
...
# Just reproduce Data_size simpler version of numpy intersect1d (not numba supported
# yet)
@numba.njit()
...
def sparse_ll_dirichlet(ind1, data1, ind2, data2):
    # The probability of rolling data2 in sum(data2) trials on Data_size die that rolled data1 in sum(data1) trials
    n1 = np.sum(data1)
    n2 = np.sum(data2)
